@extends('layout')
@section('content')
    <form action="{{route('roles.update',$role->id)}}" method="POST" class=" bg-white shadow  p-4 py-40" >
        @csrf
        @method('PUT')
        ...
        <div class="editor mx-auto w-11/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
            <textarea class="description bg-gray-100 sec p-3 h-20 border border-gray-300 outline-none" spellcheck="false" name="name" required >{{$role->name}}</textarea>
