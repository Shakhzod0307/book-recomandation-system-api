@extends('layout')
@section('content')
    <form action="{{route('books.update', $book->id)}}" method="POST" enctype="multipart/form-data" class="bg-white shadow p-4 py-8" x-data="{ images: [] }">
        @csrf
        @method('PUT')
        <div class="heading text-center font-bold text-2xl m-5 text-gray-800 bg-white">Update Book</div>
        @if(session('success'))
        ...
        <div class="editor mx-auto w-10/12 flex flex-col text-gray-800 border border-gray-300 p-4 shadow-lg max-w-2xl">
            <textarea class="description bg-gray-100 sec p-3 h-20 mb-4 border border-gray-300 outline-none" spellcheck="false" name="title" required>{{$book->title}}</textarea>
            <div class="relative inline-flex mb-4">
                ...
                <select name="genre" class="border border-gray-300 rounded-full text-gray-600 h-10 pl-5 pr-10 bg-white hover:border-gray-400 focus:outline-none appearance-none">
                    <option value="{{ $book->genre->id }}" selected>{{$book->genre->category}}</option>
                    @foreach($genres as $genre)
                        <option value="{{ $genre->id }}">{{ $genre->category }}</option>
                    @endforeach
                </select>
            </div>
            <textarea class="description bg-gray-100 sec p-3 h-10 mb-4 border border-gray-300 outline-none" spellcheck="false" name="page_number" required>{{$book->page_number}}</textarea>

            <textarea class="description bg-gray-100 sec p-3 h-40 mb-4 border border-gray-300 outline-none" spellcheck="false" name="description" required>{{$book->description}}</textarea>
            <div class="mb-4">
                <label class="block mb-2 font-semibold">Current Image:</label>
                <img src="{{Storage::url($book->image)}}" alt="Current Image" class="w-32 h-32 object-cover rounded mb-4">
            </div>
